package hugoandrade.euro2016backend.presenter;
...
import hugoandrade.euro2016backend.common.GenericPresenter;
import hugoandrade.euro2016backend.object.Country;
import hugoandrade.euro2016backend.object.Match;
import hugoandrade.euro2016backend.model.MainModel;
...
public class MainPresenter
    ...
    public void reportUpdateMatchRequestResult(String message, Match match) {
        ...
        if (match != null) {
            for (int i = 0 ; i < allMatches.size() ; i++)
                if (allMatches.get(i).matchNo == match.matchNo) {
    ...
    private void updateAllGroups() {
        ...
        new AsyncTask<Void, Void, HashMap<String, ArrayList<Country>> >() {
            ...
            protected HashMap<String, ArrayList<Country>>  doInBackground(Void... params) {
                ...
                // of the knockout stage appropriately
    ...
    private void updateCountries() {
        ...
        for (int i = 0; i < countryNameArray.length; i++) {
            ...
            for (Match match : getGroupStageMatches(allMatches)) {
                ...
                if (match.homeTeam.equals(countryNameArray[i])) {
                    goalsForList.add(match.homeTeamGoals);
                    goalsAgainstList.add(match.awayTeamGoals);
                    opponentList.add(match.awayTeam);
                }
                else if (match.awayTeam.equals(countryNameArray[i])) {
                    goalsForList.add(match.awayTeamGoals);
                    goalsAgainstList.add(match.homeTeamGoals);
                    opponentList.add(match.homeTeam);
                }
            }
            ...
            if (allGroups.containsKey(t.group)) {
                allGroups.get(t.group).add(t);
            } else {
                allGroups.put(t.group, new ArrayList<Country>());
                allGroups.get(t.group).add(t);
            }
        }
    }
    ...
    private void updateKnockOutMatchUpWhenAllGroupStageMatchesHaveBeenPlayed(HashMap<String, ArrayList<Country>> allGroups) {
        ...
        Collections.sort(bestThirdPlaceTeams, new Comparator<Country>() {
            ...
            public int compare(Country lhs, Country rhs) {
                return lhs.group.toCharArray()[0] - rhs.group.toCharArray()[0];
            }
        });
        ArrayList<String> bestThirdPlaceGroups = new ArrayList<>();
        bestThirdPlaceGroups.add(bestThirdPlaceTeams.get(0).group);
        bestThirdPlaceGroups.add(bestThirdPlaceTeams.get(1).group);
        bestThirdPlaceGroups.add(bestThirdPlaceTeams.get(2).group);
        bestThirdPlaceGroups.add(bestThirdPlaceTeams.get(3).group);
        ...
        if (bestThirdPlaceGroups.contains("A")){ //A - - -
            if (bestThirdPlaceGroups.contains("C") && bestThirdPlaceGroups.contains("D")) { //A-C-D-
                if (bestThirdPlaceGroups.contains("B")) { //A-B-C-D
                    updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WA vs 3C
                    updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WB vs 3D
                    updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WC vs 3A
                    updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WD vs 3B
                }
                else { //A-C-D-*(E/F)
                    updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WA vs 3C
                    updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WB vs 3D
                    updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WC vs 3A
                    updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WD vs 3(E/F)
                }
            }
            else { // A-B-*(C/D/E)-*(E/F)
                if (bestThirdPlaceGroups.contains("B")) {
                    updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WA vs 3(C-D-E)
                    updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WB vs 3A
                    updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WC vs 3B
                    updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WD vs 3(E-F) (always a group later than vs WA)
                }
                else { // A-*(C/D)-E-F
                    updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WA vs 3(C-D)
                    updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WB vs 3A
                    updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WC vs 3E
                    updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WD vs 3F
                }
            }
        }
            ...
            if (bestThirdPlaceGroups.contains("C") && bestThirdPlaceGroups.contains("D")) { //B-C-D-*(E/F)
                updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WA vs 3C
                updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WB vs 3D
                updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WC vs 3B
                updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WD vs 3(E-F)
            }
            else {//B-*(C/D)-E-F
                updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WA vs 3E
                updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WB vs 3(C-D)
                updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WC vs 3B
                updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WD vs 3F
            }
        } else { //C-D-E-F
            updateMatchUpInCloud(allMatches.get(40 - 1), bestThirdPlaceTeams.get(0).name, "AWAY"); // WA vs 3C
            updateMatchUpInCloud(allMatches.get(38 - 1), bestThirdPlaceTeams.get(1).name, "AWAY"); // WB vs 3D
            updateMatchUpInCloud(allMatches.get(41 - 1), bestThirdPlaceTeams.get(3).name, "AWAY"); // WC vs 3E
            updateMatchUpInCloud(allMatches.get(39 - 1), bestThirdPlaceTeams.get(2).name, "AWAY"); // WD vs 3F
        }
    }
    ...
    private void updateKnockOutMatchUpWhenAllGroupMatchesHaveBeenPlayed(ArrayList<Country> countries,
        ...
        // Update knockout stage matches only if necessary
        switch (group) {
            case "A": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(40 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(37 - 1), countries.get(1).name, "HOME");
                } else {
            ...
            case "B": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(38 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(44 - 1), countries.get(1).name, "HOME");
                } else {
            ...
            case "C": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(41 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(37 - 1), countries.get(1).name, "AWAY");
                } else {
            ...
            case "D": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(39 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(43 - 1), countries.get(1).name, "AWAY");
                } else {
            ...
            case "E": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(43 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(42 - 1), countries.get(1).name, "AWAY");
                } else {
            ...
            case "F": {
                if (areAllMatchesPlayed) {
                    updateMatchUpInCloud(allMatches.get(42 - 1), countries.get(0).name, "HOME");
                    updateMatchUpInCloud(allMatches.get(44 - 1), countries.get(1).name, "AWAY");
                } else {
    ...
    private void putCountriesInDatabase(HashMap<String, ArrayList<Country>> allGroups) {
        for (Country country : allGroups.get("A"))
            if (!country.equalsInstance(allCountries.get(country.name))) {
                allCountries.get(country.name).set(country);
                getModel().updateCountry(allCountries.get(country.name));
            }
    ...
    private void updateMatchUpInCloud(Match match, String country, String matchUpPosition) {
        ...
        if (matchUpPosition.equals("HOME") && match.homeTeam.equals(country))
            return;
        if (matchUpPosition.equals("AWAY") && match.awayTeam.equals(country))
            return;
    ...
    private ArrayList<Country> orderGroup(ArrayList<Country> group) {
        ...
        for (int i = 1 ; i < 4 ; i++) {
            ...
            if (group.get(i - 1).points == group.get(i).points) {
        ...
        for (int i = 0 ; i < sortedGroup.size() ; i++)
            sortedGroup.get(i).position = i + 1;
    ...
    private ArrayList<Country> computeTieBreaker(ArrayList<Country> countriesTiedList) {
        ...
        // Two Countries that were tied. Compute tiebreaker between two teams;
            ...
            cloneCountriesTiedList.get(0).updateStatsFilterByCountry(
                    new ArrayList<>(Collections.singleton(cloneCountriesTiedList.get(1).name)));
            cloneCountriesTiedList.get(1).updateStatsFilterByCountry(
                    new ArrayList<>(Collections.singleton(cloneCountriesTiedList.get(0).name)));
            ...
            cloneCountriesTiedList.get(0).updateStatsFilterByCountry(
                    new ArrayList<>(Arrays.asList(
                            cloneCountriesTiedList.get(1).name,
                            cloneCountriesTiedList.get(2).name)));
            cloneCountriesTiedList.get(1).updateStatsFilterByCountry(
                    new ArrayList<>(Arrays.asList(
                            cloneCountriesTiedList.get(0).name,
                            cloneCountriesTiedList.get(2).name)));
    ...
    private static ArrayList<Match> getGroupStageMatches(ArrayList<Match> allMatches) {
        ...
        for (Match match : allMatches)
            if (match.stage.equals("Group Stage"))
